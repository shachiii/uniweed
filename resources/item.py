...
from predict_weed_rice import predict_rice_weed
import base64
import re
...
class Item(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if(data['crop'].lower()=='soybean'):
                result = predict(filename, data['language'].lower())
            elif (data['crop'].lower()=='rice'):
                result = predict_rice_weed(filename, data['language'])
                # result = predict_rice_weed(filename)
            else:
